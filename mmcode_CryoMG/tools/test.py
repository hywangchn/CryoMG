...
def main():
    ...
    if not distributed:
        ...
        outputs, featureRecord = single_gpu_test(model, data_loader, args.show, args.show_dir,
                                  **show_kwargs)
        import pandas as pd
        temp = {}
        for i in range(len(data_loader.dataset.data_infos)):
            temp[data_loader.dataset.data_infos[i]['img_info']['filename'].split('\\')[-1][:-4]] = featureRecord[i]
        fea = pd.DataFrame(temp).T
        fea.to_csv('./new_feature.csv')
    else:
