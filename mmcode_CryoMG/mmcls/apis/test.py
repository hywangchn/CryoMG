...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            result, featureExtracting = model(return_loss=False, **data)
        ##################################################
        featureExtracting = featureExtracting.cpu().numpy()
        if featureRecord is None:
            featureRecord = featureExtracting
        else:
            featureRecord = np.concatenate([featureRecord, featureExtracting], axis=0)
        ##################################################
